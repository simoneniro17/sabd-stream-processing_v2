...
public class Query3 implements MapFunction<OutlierResult, ClusteredResult> {
    
    private final double EPSILON = 5.0;
    private final int MINPTS = 3;

    @Override
    public ClusteredResult map(OutlierResult outlierResult) throws Exception {
        List<OutlierPoint> points = outlierResult.outlierPoints;

        if (points == null || points.isEmpty()) {
            // Nessun punto, ritorna risultato vuoto
            return new ClusteredResult(outlierResult.batchId, outlierResult.printId, outlierResult.tileId, Collections.emptyList());
        }
        ...
        for (List<OutlierPoint> clusterPoints : clustersMap.values()) {
            ...
            c.points.addAll(clusterPoints);
            clusters.add(c);
        }

        // Ritorna il risultato raggruppato
        return new ClusteredResult(outlierResult.batchId, outlierResult.printId, outlierResult.tileId, clusters);
    }

    public static class Cluster {
        public List<OutlierPoint> points = new ArrayList<>();
    }
}
